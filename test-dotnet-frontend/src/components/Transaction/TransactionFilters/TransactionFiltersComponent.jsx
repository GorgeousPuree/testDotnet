...
const TransactionFiltersComponent = () => {
	...
	const selectOptions = [
		...
		{
			label: 'Types',
			options: AppConstants.TRANSACTION_TYPES.map((type) => ({
	...
	const handleSelectBlur = () => {
		...
		filters.forEach((filter) => {
			if (AppConstants.TRANSACTION_STATUSES.includes(filter.label))
				statuses.push(filter.label);
			else if (AppConstants.TRANSACTION_TYPES.includes(filter.label))
				types.push(filter.label);
		});
